...
struct intf {
	int sock_req, sock_arp, sock_udp;
	char *ifn;
...
int main(int argc, char **argv) {
	...
	int proc_arp, sock_udp, sock_max = 0, dhcp_max = -1;
	unsigned int ssiz;
	unsigned char *t;
	...
	FD_ZERO(&so_set); FD_SET(sock_udp, &so_set);

	/* relay interfaces */

	bzero(relayds, RELAY_LEN * sizeof(struct intf));
	for (int i = 0; (i < RELAY_LEN) && ((i + 1) < argc); ++i) {
		relayds[i].ifn = argv[i + 1]; p = relayds[i].ifn; dhcp_max = i;
	...
	while (1) {
		...
		if (FD_ISSET(sock_udp, &re_set)) {
			...
			if ((rlen > 42) && (dhcp_max >= 0)) {
				clip = inet_ntoa(sain.sin_addr);

				printf("dhcp recv .. [%d][%s]\n", rlen, clip);

				if (strcmp(clip, "0.0.0.0") == 0) {
					send_dhcp(&(relayds[dhcp_max]), &dhcp, rlen, clip, 'f');
				} else {
					for (int i = 0; i < RELAY_LEN; ++i) {
						if ((relayds[i].ifn == NULL) || (i == dhcp_max)) { continue; }
